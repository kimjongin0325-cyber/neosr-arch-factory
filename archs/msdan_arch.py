...
class msdan(nn.Module):
    ...
    def forward(self, x0):
        ...
        if self.upscale_factor == 2:
            ...
            x_up = F.interpolate(x0,scale_factor=self.upscale=2, mode="bicubic")  # x2
        if self.upscale_factor == 3:
            x = self.upsampler(x)  # x3
            x_up = F.interpolate(x0,scale_factor=self.upscale=3, mode="bicubic")  # x3
        if self.upscale_factor == 4:
            x = self.upsampler(x)  # x4
            x_up = F.interpolate(x0,scale_factor=self.upscale=4, mode="bicubic")  # x4
